...
class Network {
  final String _url = 'https://medrese.uk/api';
  final String baseUrl = 'https://medrese.uk';
  dynamic token;
  ...
  // }
  postAuthImage(apiUrl, data, File image, imageName, String apikeyImage,
    ...
    var response = await dio.post(
      ...
      data: formData,
      options:
          Options(validateStatus: (status) => true, headers: _setHeadersAuth()),
    );
    return response.data;
  }

  postImage(apiUrl, data, File image, imageName, String apikeyImage) async {
    var fullUrl = _url + apiUrl;
    List<int> imageData = image.readAsBytesSync();
    MultipartFile multipartFile = MultipartFile.fromBytes(
      imageData,
  ...
  postAuthData(data, apiUrl, String? accessToken) async {
    ...
    final response = await http.post(Uri.parse(fullUrl),
        body: jsonEncode(data), headers: _setHeadersAuth());
  ...
  postData(data, apiUrl) async {
    ...
    final response = await http.post(Uri.parse(fullUrl),
        body: data1, headers: _setHeaders());
    return jsonDecode(response.body);
  }
  ...
  getAuthData(apiUrl, String? accessToken) async {
    ...
    await _getToken(accessToken);
    final response =
        await http.get(Uri.parse(fullUrl), headers: _setHeadersAuth());
    return jsonDecode(response.body);
  }
